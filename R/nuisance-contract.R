...
validate_nuisance <- function(problem, spec, bayes_enabled = FALSE) {
  ...
  checkmate::assert_list(nu, names = "named", .var.name = "nuisance_estimates")
  ...
  for(f in contract$fields) {
    ...
    if(is.null(x)) {
      if(f$required) {
        stop("nuisance_estimates$", f$name, " is required for estimand '", problem$estimand,
    ...
    if(!identical(as.integer(got), f$dim)) {
      stop("nuisance_estimates$", f$name, " has shape(", paste(got, collapse = ", "),
           "); expected (", paste(f$dim, collapse = ", "), ").", call. = FALSE)
    }

    v <- as.numeric(x)
    if(anyNA(v) || !all(is.finite(v))) {
      stop("nuisance_estimates$", f$name, " contains missing or non-finite values.", call. = FALSE)
    }
    if(!is.null(f$lower) && min(v) < f$lower) {
      nuisance_problem(f$severity, "nuisance_estimates$", f$name, " must be > ", f$lower, "; observed minimum ", signif(min(v), 4), ".")
    }
    if(!is.null(f$upper) && max(v) > f$upper) {
      nuisance_problem(f$severity, "nuisance_estimates$", f$name, " must be < ", f$upper, "; observed maximum ", signif(max(v), 4), ".")
    }
  }
