test_that("calculate_K returns correct fluctuation covariate", {
  ...
  psi <- torch::torch_tensor(rep(1, 5e2))
  beta <- torch::torch_tensor(c(1, 1), requires_grad = TRUE)
  design_matrix <- torch::torch_tensor(matrix(1, ncol = 2, nrow = 5e2))

  K <- calculate_K(Lm(loss, working_model), psi, beta, design_matrix, Minv)
